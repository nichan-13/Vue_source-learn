...
// init()函数，初始化patch函数
var patch = (0, _snabbdom.init)([]); // h() 函数，用来创建VNode
...
}, 'Hello World'); // 获取页面的真实dom

var app = document.querySelector('#app'); // patch() 函数，用来更新dom
// 将两个VNode进行对比，把对比的差异渲染到页面中（更新到真实dom中）并返回一个VNode
// 第一个参数：可以是dom元素，内部会把dom元素转换成VNode（会把真实dom转换为虚拟dom）
// 第二个参数：VNode
// 返回值：VNode

var oldVnode = patch(app, vnode); // Hello World
// 假设的时刻

vnode = (0, _snabbdom.h)('div', 'Hello Snabbdom'); // patch()函数对比后更新
